...
from afadvo.nn.layers.agc import AFConv

from utils.visualize import visualize_tsne

class VAGCEncoder(torch.nn.Module):
    def __init__(self, n_node_atts, n_edge_atts, emb_dim):
        super(VAGCEncoder, self).__init__()
        self.conv1 = AFConv(n_node_atts, n_edge_atts, 2 * emb_dim)
        self.conv2 = AFConv(2 * emb_dim, 2 * emb_dim, 4 * emb_dim)
        self.conv3 = AFConv(4 * emb_dim, 4 * emb_dim, 4 * emb_dim)
        self.conv4 = AFConv(4 * emb_dim, 4 * emb_dim, 2 * emb_dim)
        self.conv_mu = AFConv(2 * emb_dim, 2 * emb_dim, emb_dim)
        self.conv_logvar = AFConv(2 * emb_dim, 2 * emb_dim, emb_dim)
...
class VGAEDecoder(torch.nn.Module):
    def __init__(self, in_dim, out_dim, edge_weight):
        ...
        self.conv1 = AFConv(in_dim, 2 * in_dim)
        self.conv2 = AFConv(2 * in_dim, 2 * out_dim)
        self.conv3 = AFConv(2 * out_dim, out_dim)
...
#         self.preprocess(scale=scale, normalize=normalize)
        ...
        # set model
        self.model = torch_geometric.nn.VGAE(VAGCEncoder(self.n_node_atts, self.dim))
...
# AGC deal with multi-dimensional edge attributes, so no need to convert to scalar
